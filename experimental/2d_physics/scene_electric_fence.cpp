...
Scene::Scene() : sim::Scene(b2Vec2(0, -9.8f), sim::Rect(-100, -100, 200, 200)) {
  ...
  // agitator (top)
  auto top_agitator = sim::addCross(0, 0, 45, 2, &world_);
  ...
  auto bottom_left_agitator = sim::addCross(0, 0, 45, 2, &world_);
  ...
  auto bottom_right_agitator = sim::addCross(0, 0, 45, 2, &world_);
  ...
  for (int i = 0; i < 500; ++i) {
    script_.record(i / 30.0f, [&, i](float) {
      sim::addBullet(0, 0, 100 * (i % 2 ? 100 : -100), (i - 250) * 50, &world_);
    });
  }
}
